#!/usr/bin/env python
# encoding: utf-8
# Généré par Mocodo 2.3.7 le Thu, 23 Dec 2021 09:09:38
...
with codecs.open('Conferences_geo.json') as f:
...
leg=straight_leg_factory(ex,ey,37,59,x,y,29,25,21+2*card_margin,15+2*card_margin)
...
(ex,ey) = (cx[u"CONFERENCE"],cy[u"CONFERENCE"])
leg=straight_leg_factory(ex,ey,49,51,x,y,29,25,22+2*card_margin,15+2*card_margin)
lines += u"""\n<line x1="%(ex)s" y1="%(ey)s" x2="%(ax)s" y2="%(ay)s" stroke="%(stroke_color)s" stroke-width="2"/>""" % {'ex': ex, 'ey': ey, 'ax': x, 'ay': y, 'stroke_color': colors['leg_stroke_color']}
(tx,ty)=offset(*leg.card_pos(False,shift[u"Can be,CONFERENCE"]))
...
lines += u"""\n	</g>""" % {}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">COMMENT</text>""" % {'x': -32+x, 'y': -41.3+y, 'text_color': colors['entity_cartouche_text_color']}
...
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">author</text>""" % {'x': -32+x, 'y': 0.8+y, 'text_color': colors['entity_attribute_text_color']}
...
lines += u"""\n\n<!-- Entity CONFERENCE -->"""
(x,y) = (cx[u"CONFERENCE"],cy[u"CONFERENCE"])
lines += u"""\n<g id="entity-CONFERENCE">""" % {}
lines += u"""\n	<g id="frame-CONFERENCE">""" % {}
...
lines += u"""\n	</g>""" % {}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">CONFERENCE</text>""" % {'x': -42+x, 'y': -33.3+y, 'text_color': colors['entity_cartouche_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">id</text>""" % {'x': -44+x, 'y': -8.3+y, 'text_color': colors['entity_attribute_text_color']}
...
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">city</text>""" % {'x': -44+x, 'y': 8.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">year</text>""" % {'x': -44+x, 'y': 25.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">isInternational</text>""" % {'x': -44+x, 'y': 42.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n</g>""" % {}
lines += u'\n</svg>'

with codecs.open("Conferences.svg", "w", "utf8") as f:
    f.write(lines)
safe_print_for_PHP(u'Fichier de sortie "Conferences.svg" généré avec succès.')